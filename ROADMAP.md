...
| 206 | Research: /insights command integration | User surfaced 2026-04-19 via Reddit: CC has an `/insights` slash command. Questions: (a) what does it do exactly — per-session token/cost analytics? model-performance signals? (b) is it official CC or a community plugin? (c) does its output expose cache-hit ratios / degradation signals the wizard could consume (feeds #96 benchmark audit and #204 cache-cost guardrail)? (d) can the wizard recommend it in the setup flow as a complementary command? (e) if `/insights` is a native CC command, our telemetry-scraping roadmap items are partially pre-empted — reassess scope. First check: source the feature (`/help insights` in CC, anthropics/claude-code commits, claude.com/docs), then decide integration strategy |
...
| 208 | Compact-at-seams PreCompact hook | User raised 2026-04-19: "when to compact is more about seams than %". Threshold alone (25-30% remaining ≈ 70% used) is naive — compacting mid-Codex-round loses round-1 evidence + certify conditions that round-2 needs to reference. Fix: add a PreCompact hook that reads `.reviews/handoff.json` status; if `PENDING_REVIEW` or `PENDING_RECHECK`, emit `"HOLD: mid-Codex-round, compact after CERTIFIED"`. Also checks TodoWrite state — if any task is `in_progress`, emit "HOLD: finish current todo first". Percentage stays as the trigger; the hook's job is to veto when we're on a non-seam. Seam taxonomy to document: commit boundary, Codex CERTIFIED, PR merge, ROADMAP item DONE. Quality test: seed handoff.json with `PENDING_RECHECK`, assert PreCompact hook emits HOLD; flip to `CERTIFIED`, assert hook silent. Complements #204 (cache-cost guardrail) — both are hooks that observe session state and warn on anti-patterns |
