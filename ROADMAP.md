...
| 194 | opencode SDLC Adapter (fully OSS stack) | Extend the adapter pattern (Codex adapter in #91 is the proof) to [SST/opencode](https://github.com/sst/opencode) so the SDLC wizard works with open-source model backends (local Llama/Qwen via Ollama, hosted OSS via Together/Groq, etc.). Value: completely free, privacy-preserving, no Anthropic/OpenAI lock-in. Scope per adapter: (1) map hooks → opencode's pre/post-tool primitives, (2) port instructions-loaded-check + sdlc-prompt-check + tdd-pretool-check, (3) port skills (SKILL.md is already portable per #91), (4) install.sh with non-destructive config merge (mirror codex-sdlc-wizard structure), (5) quality tests proving enforcement actually fires under opencode. Sibling repo: `BaseInfinity/opencode-sdlc-wizard`. Prove-It Gate: must score an E2E run on opencode+Llama stack and compare to Claude Code baseline — if enforcement holds, this unblocks an entire OSS audience |
