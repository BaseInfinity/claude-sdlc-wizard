...
| 193 | Fix PR-branch score-push race | ci.yml `Persist scores to PR branch` uses `git push origin HEAD:refs/heads/<branch>` from a `refs/pull/N/merge` checkout. When main advances during the ~3-min CI run, the merge ref diverges and the push is rejected. `continue-on-error: true` swallows the failure silently. Result: even successful PRs don't append to `score-history.jsonl`. Fix: fetch + rebase the PR-branch tip before push; retry on conflict; surface persistent failures as warnings (not silent). Related: PR #193 fixed the CRITICAL_MISS abort; this closes the second half of the benchmarking stall |
...
| 195 | Dynamic effort auto-bump hook | The SDLC skill's confidence table says "Run `/effort max` now" on LOW/FAILED 2x/CONFUSED, but there's no enforcement — Claude still has to remember to do it mid-session. Wire a hook that watches for signals (repeated test failures in the same file, confidence statements like "LOW" or "not sure", consecutive TDD RED with no progress) and either (a) auto-bumps effort via a slash-command trigger, or (b) emits a loud user-facing nudge at session start / PreToolUse when a prior turn logged a failure pattern. Spinning at `high` when `xhigh`/`max` is warranted wastes more budget than the upgrade costs. Quality test: simulate a 2-retry fail loop in a fixture scenario, assert hook emits the bump directive |
| 196 | Wizard staleness nudge (N-releases-behind) | Current session-start hook nudges only when `weekly-update.yml` has already opened an auto-update PR. Gap: if the consumer repo skipped 3+ releases without opening a PR, or never set up weekly-update, no nudge fires. Fix: `instructions-loaded-check.sh` parses `<!-- SDLC Wizard Version: X.Y.Z -->` from SDLC.md, fetches latest wizard version from GitHub (cached daily), and prints a stronger nudge if installed is ≥3 minor versions behind. Related to PR #192's session-start nudge and user feedback on 2026-04-18 ("didn't realize I hadn't run /update in so long"). Quality test: fixture with version `1.25.0` while latest is `1.34.0` asserts the hook prints a "9 versions behind" nudge |
