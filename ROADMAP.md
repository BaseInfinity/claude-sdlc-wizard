...
| 52 | Confidence-Driven Setup (Kill the 18 Questions) | The 18-question setup is bad UX. Fix: scan repo first, build confidence per data point. Only ask questions until aggregate confidence hits 95%. If the scan gives us 95% confidence on test framework, src dir, commands, etc — don't ask, just fill it in. Only surface questions where the scan can't infer the answer (preferences like testing philosophy, response detail level). The number of questions should be DYNAMIC — could be 2, could be 8, depends on how much the scan resolves. AI infers what it can to increase confidence, asks humans to fill gaps it can't. Proven viable: snowcone install correctly inferred 15/16 answers from scan alone. This is the #1 friction point for new repo onboarding |
