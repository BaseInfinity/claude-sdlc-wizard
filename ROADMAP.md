...
| 24 | Harness Design for Long-Running Tasks | Investigate Anthropic's [harness design patterns](https://www.anthropic.com/engineering/harness-design-long-running-apps) for adoption: GAN-style generator/evaluator separation, sprint-based decomposition, contract negotiation before implementation, file-based artifact handoffs between agent instances, calibrated evaluator scoring rubrics. Key question: can the SDLC skill's self-review + cross-model review loop evolve into a structured harness (dedicated evaluator agent with explicit criteria, context resets between sprints, planner→generator→evaluator pipeline)? Test on a multi-hour feature build and compare output quality vs current single-agent SDLC loop |
| 25 | Codex README/Docs Audit | Have Codex CLI audit README.md and other user-facing docs to better match project verbiage and tone. Not a code change — a polish pass on documentation voice |
