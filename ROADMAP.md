...
| 5 | 53 | Plan Auto-Approval Gate | If confidence >= 95% AND complexity is low/medium, skip plan approval step and go straight to TDD. Reduces friction for established patterns. Could be a setup wizard question or SDLC.md setting |
| 6 | 55 | Debugging Methodology | Research: does the wizard need a dedicated debugging section? Currently covered implicitly by TDD + SDET philosophy. Missing: systematic investigation methodology, `git bisect`, environment-specific debugging |
| 7 | 37 | `/feedback` — Community Contribution Loop | Single `/feedback` command: bug reports, feature requests, pattern sharing. Diffs user's SDLC against wizard defaults, creates GH issue or PR on wizard repo. Research: privacy, CI cost, contribution quality |
| 8 | 44 | BRANDING.md Detection & Guidance | Research: detect/suggest BRANDING.md (brand voice, tone, naming conventions). Options: always create, detect from assets, suggest only when UI/content work detected |
| 9 | 32 | N-Reviewer CI Pipeline | Support N parallel code reviewers (Claude, Codex, etc.) commenting on PRs. Agent responds like a dev to each reviewer, iterates until all approve |
| 10 | 45 | `/agents` Subagent Exploration | Research: explore Claude Code's `.claude/agents/` (custom subagents). Can an `sdlc-reviewer` or `ci-debug` agent add value beyond hooks+skills? |
