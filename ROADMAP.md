...
| 5 | 37 | `/feedback` — Community Contribution Loop | **Research needed.** Single `/feedback` command handles all types: bug reports, feature requests, pattern sharing, SDLC improvements. Asks permission to scan repo, diffs user's SDLC against wizard defaults (custom hooks, modified philosophies, unique test patterns, CLAUDE.md learnings). Identifies what's worth contributing back — different projects optimize differently but core philosophy improvements help everyone. Creates GH issue or PR on wizard repo. Research areas: (1) privacy — what's safe to scan/share, permission model, (2) CI cost — external PRs trigger CI, need budget protection or manual-trigger-only for external contributions, (3) contribution quality — how to filter signal from noise, (4) manual vs auto — even manual "here's what I changed and why" issues are valuable. Subsumes original #37 (Lesson Contribution Hook). Inspired by Claude Code's `/feedback` but broader — not just bugs, but sharing what works |
| 6 | 44 | BRANDING.md Detection & Guidance | Research: should the wizard detect/suggest a BRANDING.md (brand voice, tone, naming conventions, visual identity guidelines)? Options: always create, detect from existing assets (logos, style guides, design system), suggest only when UI/content work detected |
| 7 | 32 | N-Reviewer CI Pipeline | Support N parallel code reviewers (Claude, Codex, etc.) commenting on PRs. Agent responds like a dev to each reviewer, iterates until all approve. Depends on Codex experiment (#27) providing data first |
| 8 | 45 | `/agents` Subagent Exploration | Research: explore Claude Code's `/agents` (custom subagents in `.claude/agents/`). Can an `sdlc-reviewer` or `ci-debug` agent add value beyond hooks+skills? Testing story for agent behavior |
...
| 55 | Debugging Methodology Section | Research: does the wizard need a dedicated debugging section? Currently covered implicitly (TDD: bug→failing test→fix, SDET philosophy, CONFUSED/FAILED 2x escalation, flaky test investigation). Missing: systematic investigation methodology (reproduce→isolate→root cause→fix→regression test), `git bisect` for regressions, environment-specific debugging (local/staging/prod), when to use logging vs stepping. Prove It Gate: can we prove a debugging section improves outcomes? TDD already enforces the key discipline (write the failing test first). May be unnecessary bloat — research whether users struggle with debugging workflow or if TDD covers it |
