...
| Priority | # | Item | Description |
|----------|---|------|-------------|
| 1 | -- | CC Version Check + Weekly-Update Audit | Audit why we were 9 CC versions behind (2.1.81 vs 2.1.90). Add CC version check to `instructions-loaded-check.sh` so users (and us) get notified. Verify weekly-update workflow is firing correctly |
| 2 | 75 | Research: Post-Mortem Frameworks | Evaluate SBAR, 5 Whys, blameless post-mortems for AI agent SDLC. Formalize "every mistake becomes a rule" pattern |
| 3 | 68 | Hook `if` Conditionals | Use CC v2.1.85+ `if` field to reduce false positives. Requires CC update first |
| 4 | 78 | E2E Fixture: Firmware/Embedded Domain | Prove wizard handles non-web domains (spruceOS-style). Evidence: arkun (Discord) |
| 5 | 82 | Research: Domain-Specific *DLC Variants | Legal (LDLC), Firmware (FDLC), Research (RDLC), Content (CDLC) domain detection |
| 6 | 80 | Research: SDLC Effectiveness Scoreboard | Track catches per layer, prove ROI |
...
| 87 | Research: Plugin/Marketplace Distribution | Casual users browse Claude app skills marketplace, Codex plugins, CC `/powerup` discovery — but `agentic-sdlc-wizard` isn't listed anywhere. Currently only discoverable via npm (`npx agentic-sdlc-wizard init`) or GitHub search. Research: (1) How to submit to Anthropic's knowledge-work-plugins / `.claude-plugin` format (connects to #66), (2) How to get listed in Claude app's skills/plugins marketplace, (3) Codex agents marketplace if it exists, (4) SEO/discoverability for `npx` packages. Question: does plugin format give us marketplace listing for free, or is there a separate submission process? Reference: #66 (Convert to Plugin Format), #62 (Bidirectional Plugin Ecosystem) |
