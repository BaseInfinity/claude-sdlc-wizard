...
| 94 | Research: Advisor Tool Strategy + A/B Test | **ACTIVE PIPELINE DECISION:** Sonnet executor → Opus advisor/reviewer → Codex xhigh cross-review. Currently approximated manually (Sonnet subagents, Opus main, Codex CLI). Formal advisor tool (`advisor-tool-2026-03-01`) is API-only beta — adopt natively when CC CLI integrates it. **Prove It Gate:** must A/B test against current Opus-only pipeline using our own E2E harness before recommending to wizard users. Test matrix: Config A (current): Opus → Codex xhigh. Config B (advisor): Sonnet + Opus → Codex xhigh. Config C (full): Sonnet + Opus → Codex xhigh + Opus on Codex output. Config D (budget): Sonnet → Codex xhigh. **Measure:** evaluate.sh scores, catch rate (catches.jsonl), cost per session. Sonnet can also apply Codex review fixes (reviewer doesn't need to be the fixer). Blog: `claude.com/blog/the-advisor-strategy`. Docs: `platform.claude.com/docs/en/managed-agents/advisor`. Note: API-billed, not Max |
