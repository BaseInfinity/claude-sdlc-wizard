...
| 195 | ~~Dynamic effort auto-bump hook~~ DONE | `sdlc-prompt-check.sh` now reads the UserPromptSubmit payload, scans for LOW/FAILED/CONFUSED trigger phrases (`low confidence`, `i'm stuck`, `still failing`, `failed again`, `keeps failing`, `tried twice`, `not sure why`, `can't figure`, `confused`), and logs a timestamped signal to `$SDLC_WIZARD_CACHE_DIR/effort-signals.log`. When ≥2 signals land inside a 30-minute window, the hook emits a loud `!! EFFORT BUMP REQUIRED !!` block with the exact `/effort xhigh` command. 5 quality tests: writes-signal on trigger, no-signal on neutral prompt, nudge-on-2-recent, silent-on-1, old-signals-ignored. Negative control proved the AND requirement (marker + command) both gate the bump test. Enforces the SDLC confidence table mid-session |
