...
| 5 | 181 | Dual-Channel Install Drift Guardrails (Option A) | Users installing via both npm CLI (`npx agentic-sdlc-wizard init`) AND Claude plugin (`/plugin install sdlc-wizard`) end up with duplicate `/update-wizard` commands and version drift between channels. Option A: keep both channels, add drift detection. (a) `cli/init.js` + `cli/update.js` detect plugin install paths (`~/.claude/plugins-local/sdlc-wizard-wrap/`, `~/.claude/plugins/cache/sdlc-wizard-local/`) and warn with `--force` bypass. (b) `instructions-loaded-check.sh` detects both channels active and nudges user to pick one. Docs updated with "pick one channel" guidance |
