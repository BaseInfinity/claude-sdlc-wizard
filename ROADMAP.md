...
| 94 | Research: Anthropic Advisor Tool Strategy | Anthropic released advisor tool beta (`advisor-tool-2026-03-01`): pairs a faster executor model (Sonnet) with a stronger advisor (Opus) for strategic guidance mid-generation. Executor codes, advisor plans — 400-700 token advice bursts. **Evaluate:** (a) Sonnet executor + Opus advisor vs our current Claude+Codex cross-model review, (b) could advisor pattern replace/supplement Codex review step? (c) cost comparison (advisor = Opus sub-inference per call vs codex exec full review). Blog: `claude.com/blog/the-advisor-strategy`. Docs: `platform.claude.com/docs/en/managed-agents/advisor`. Key: advisor is API-billed, not Max — budget implications. Also test: Sonnet executor + Codex as advisor (if possible). **This is a potential paradigm shift for our review pipeline** |
