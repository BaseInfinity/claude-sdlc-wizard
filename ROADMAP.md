...
| Priority | # | Item | Description |
|----------|---|------|-------------|
| 1 | 93 | Node.js 20 EOL | **Hard deadline June 2, 2026.** `actions/checkout@v4` + `oven-sh/setup-bun` forced to Node 24. Audit CI workflows, update matrix, test. Moved from Back Burner — time-sensitive |
| 2 | 88 | Autocompact in Setup | Detect model context window during `/setup-wizard`, set `CLAUDE_AUTOCOMPACT_PCT_OVERRIDE` in generated settings.json. Recommended: 75% for 200K, 30% for 1M. Research complete (#92 benchmarking methodology shipped) |
...
| Priority | # | Item | Description |
|----------|---|------|-------------|
| 1 | 74 | Watercooler Index | Audit Stefan's articles from softwareautomation.notion.site. Extract testing philosophies, SDET insights, automation opinions. Compare against wizard guidance. Research phase |
| 2 | 75 | Post-Mortems | Evaluate SBAR, 5 Whys, blameless post-mortems for AI agent SDLC. Does formalizing beyond "every mistake becomes a rule" add value? Prove It Gate applies. Research phase |
| 3 | 78 | Firmware Fixture | Prove wizard handles non-web domains. Python + shell SD card overlay with multi-device configs. Validates domain-adaptive setup (#79) generates correct firmware testing layers. Partially satisfied by #79 |
| 4 | 71 | KAIROS Monitor | Proactive always-on agent mode + multi-agent coordination. Watch-only until feature exits experimental. Track feature flags, prepare skill content |

Deferred: #83 (Local Model — needs real demand signal), #82 (Domain DLCs — Stefan's separate track)
