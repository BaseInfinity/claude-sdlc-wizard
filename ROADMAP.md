...
| Priority | # | Item | Description |
|----------|---|------|-------------|
| 1 | 100 | ~~API Feature Detection in Auto-Update~~ DONE | PR #184. Shepherd pattern: LLM-free weekly detector (`.github/workflows/weekly-api-update.yml`) fetches `platform.claude.com/docs/en/release-notes/api.md`, parses ATX date headers with ordinal normalizer (`scripts/parse-api-changelog.py`), opens/updates single `api-review-needed` tracking issue. State persisted via `scripts/persist-api-state.sh` with non-blocking push (branch-protection safe). Session-time hook (`instructions-loaded-check.sh`) nudges when open issues exist — gated on LOCAL workflow presence so consumer/forks only see their own detector's issues. 29 tests incl. 6 fixture-based parser tests + 2 integration tests (mocked git push rejection, mocked gh issue query). Codex xhigh 3-round: 9/10 CERTIFIED. Claude PR review: 1 P1 + 3 P2s addressed (silent-swallow removed, page-order assumption fixed via `max()`, label-create error handling scoped to `already_exists`) |
