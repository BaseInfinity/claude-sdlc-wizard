...
| 3 | 91 | Multi-Agent Adapter Layer | Codex/Cursor/Windsurf adapters. Skills port directly, hooks need per-agent adaptation. Expands reach beyond Claude Code |
...
| 96 | Audit: Claude Code Degradation Detection | **CONFIRMED REAL.** Anthropic staff (Boris Cherny) acknowledged adaptive thinking under-allocation (GH #42796, 274 comments). aistupidlevel.info shows Opus 4.6 dropped to 47 (old Opus 4.1 was 54). Root causes: adaptive thinking, medium effort default, "simplest approach" system prompt (v2.1.64). Read:Edit ratio collapsed 6.6→2.0. Community reports 200K model better than 1M. **Action:** (a) audit our E2E score history — did CUSUM detect this? (b) if not, why not? (c) add aistupidlevel as external signal to weekly-update (d) consider `/effort high` as wizard default (e) add anti-laziness instructions to CLAUDE.md template |
