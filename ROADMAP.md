...
| 94 | Research: Advisor Tool Strategy + A/B Test | Anthropic released advisor tool beta (`advisor-tool-2026-03-01`): Sonnet executor + Opus advisor for strategic guidance mid-generation. **A/B test matrix using our E2E harness:** Config A (current): Opus executor → Codex xhigh cross-review. Config B (advisor): Sonnet executor + Opus advisor → Codex xhigh cross-review. Config C (full): Sonnet + Opus advisor → Codex xhigh + Opus advisor on Codex output. Config D (baseline): Sonnet → Codex xhigh only. **Measure:** evaluate.sh scores, catch rate, cost per session. Key insight: Opus reviews Sonnet's work (advisor), Codex reviews the whole output (cross-model) — three intelligence tiers at right price points. Blog: `claude.com/blog/the-advisor-strategy`. Docs: `platform.claude.com/docs/en/managed-agents/advisor`. Note: API-billed, not Max. **Also add as wizard recommendation** — offer advisor pipeline as review option alongside Codex cross-model review |
...
| 96 | Research: Claude Code Degradation Detection | Reddit reports capability degradation (`r/ClaudeCode/comments/1sh9302/claude_code_capability_degradation_is_real`). **Question:** did our E2E/CI detect any degradation signal? Audit: (a) E2E scores over time (CUSUM trends), (b) Tier 2 trial variance, (c) weekly-update CC version test results, (d) any anomalous test failures that correlate with CC version bumps. If we didn't detect it, why not? Gap in our monitoring? |
